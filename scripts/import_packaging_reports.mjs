...
async function importData() {
  ...
  const csvPath = path.join(process.cwd(), 'public', 'files', 'packaging_reports.csv');
  ...
  for (const row of results) {
    ...
    const docRef = db.collection('packaging_reports').doc(docId);
    ...
    for (const [key, value] of Object.entries(row)) {
      ...
      if (key.startsWith('packagingBreakdown.')) {
        const nestedKey = key.split('.')[1];
        packagingBreakdown[nestedKey] = parseNumber(value);
      } else if (['__createTime', '__updateTime'].includes(key)) {
        docData[key] = parseDate(value);
      } else if (['date', 'customerName', 'remark', 'consigneeName', 'mode', 'product', 'transportMode', 'shipment'].includes(key)) {
        docData[key] = value;
      } else {
    ...
    // Convert 'date' correctly if it's a string YYYY-MM-DD
    docData.packagingBreakdown = packagingBreakdown;

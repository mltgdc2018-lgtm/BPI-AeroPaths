...
import { Copyleft as Package, Boxes } from "lucide-react";
...
export default function ReceivingPage() {
  ...
  // Computed Stats
  const pendingCount = receivingNotes.filter(n => n.status === "Pending").length;
  const todayCount = receivingNotes.filter(n => {
    const today = new Date().toISOString().slice(0, 10);
    return n.date === today;
  }).length;
  const verifiedCount = receivingNotes.filter(n => n.status === "Verified").length;
          ...
          >
            <div className="space-y-6 mt-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <GlassCard className="p-4 flex items-center justify-between bg-[#EEF2F6]/95 border border-white/80 shadow-[8px_8px_18px_rgba(166,180,200,0.28),-8px_-8px_18px_rgba(255,255,255,0.92)] hover:bg-[#272727] group transition-all duration-300">
                  <div>
                    <p className="text-[#7E5C4A] text-sm font-medium group-hover:text-[#EFD09E]/80">Awaiting Completion</p>
                    <h3 className="text-2xl font-bold text-[#272727] mt-1 group-hover:text-[#EFD09E]">{verifiedCount}</h3>
                    <p className="text-xs text-blue-500 mt-1 font-medium">Verified items</p>
                  </div>
                  <div className="p-3 bg-[#9ACD32] rounded-xl border border-[#EFD09E]/50">
            ...
            <div className="h-[480px] flex flex-col">
              ...
              <div className="pt-4 border-t border-[#D4AA7D]/25 mt-auto flex gap-3">
                ...
                <button className="flex-1 py-2.5 bg-[#272727] hover:bg-[#1f1f1f] text-[#EFD09E] rounded-lg font-medium transition-colors shadow-lg shadow-[#272727]/25 border border-[#EFD09E]/20"
                  onClick={() => { alert("สร้างใบรับสำเร็จ (Feature under development)"); setIsNewReceivingModalOpen(false); }}>
                  Create Note
                </button>
              </div>
            </div>
            ...
            <div className="h-[480px] flex flex-col">
              ...
              <div className="pt-4 border-t border-[#D4AA7D]/25 mt-auto flex gap-3">
                {selectedReceiving?.status === "Pending" && (
                  <button onClick={() => { alert(`Verified: ${selectedReceiving?.receivingNo || selectedReceiving?.id}`); setSelectedReceiving(null); }}
                ...
                {selectedReceiving?.status === "Verified" && (
                  <button onClick={() => { alert(`Completed: ${selectedReceiving?.receivingNo || selectedReceiving?.id}`); setSelectedReceiving(null); }}
                    className="flex-1 py-2.5 bg-[#272727] hover:bg-[#1f1f1f] text-[#EFD09E] rounded-lg font-medium transition-colors shadow-lg shadow-[#272727]/25 border border-[#EFD09E]/20">
                    Complete Receiving
                  </button>
                )}
